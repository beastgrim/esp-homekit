...
#warning my implementation
bool bool_endpoint_param(client_context_t *context, const char *name) {
...
void homekit_server_on_get_characteristics(client_context_t *context) {
    ...
    if (bool_endpoint_param(context, "meta"))
        format |= characteristic_format_meta;
    
    if (bool_endpoint_param(context, "perms"))
        format |= characteristic_format_perms;
    
    if (bool_endpoint_param(context, "type"))
        format |= characteristic_format_type;
    
    if (bool_endpoint_param(context, "ev"))
...
HAPStatus process_characteristics_update(client_context_t *context, const cJSON *j_ch) {
...
void add_txt(char txt_rec[128], const char *format, ...) {
    va_list arg_ptr;
...
void homekit_setup_mdns(homekit_server_t *server) {
    ...
    add_txt(txt_rec, "md=%s", model->value.string_value);
    // protocol version (required)
    add_txt(txt_rec, "pv=1.0");
    // device ID (required)
    // should be in format XX:XX:XX:XX:XX:XX, otherwise devices will ignore it
    add_txt(txt_rec, "id=%s", server->accessory_id);
    // current configuration number (required)
    add_txt(txt_rec, "c#=%d", accessory->config_number);
    ...
    //   bits 1-7 - reserved
    add_txt(txt_rec, "ff=0");
    // status flags
    //   bit 0 - not paired
    //   bit 1 - not configured to join WiFi
    //   bit 2 - problem detected on accessory
    //   bits 3-7 - reserved
    add_txt(txt_rec, "sf=%d", (server->paired) ? 0 : 1);
    // accessory category identifier
    add_txt(txt_rec, "ci=%d", accessory->category);
